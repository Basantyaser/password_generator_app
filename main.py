...
class Passwordgenerator:
    ...
    def generate(self):
        ...
        if entropy < 28:
            self.entropy_label.configure(text=f"Entropy: {entropy:.2f} bits (Very Weak)", text_color="red")
        elif 28 <= entropy < 36:
            self.entropy_label.configure(text=f"Entropy: {entropy:.2f} bits (Weak)", text_color="orange")
        elif 36 <= entropy < 60:
            self.entropy_label.configure(text=f"Entropy: {entropy:.2f} bits (Medium)", text_color="yellow")
        elif 60 <= entropy < 128:
            self.entropy_label.configure(text=f"Entropy: {entropy:.2f} bits (Strong)", text_color="green")
        else:
